...
export default function Practicalor() {
    ...
    useEffect(() => {
        const storedData = localStorage.getItem('formArray');
        if (storedData) {
            setFormArray(JSON.parse(storedData));
        }
    }, []);

    // Save formArray to localStorage whenever it changes
    useEffect(() => {
    ...
    const handleForm = (e) => {
        ...
        if (editIndex !== null) {
            const updatedArray = formArray.map((item, index) => 
    ...
    const handleEdit = (index) => {
        const entry = formArray[index];
        setRadio(entry.radio);
        setChecked1(entry.checked1);
        setChecked2(entry.checked2);
        setChecked3(entry.checked3);
        setEditIndex(index);

        // Manually setting form inputs' values
                ...
                <tbody>
                    {formArray.map((item, index) => (
                        <tr key={index}>
                            ...
                            <td style={{ border: '1px solid black', padding: '8px' }}>{item.checked1 ? 'Checked' : 'Unchecked'}</td>
                            <td style={{ border: '1px solid black', padding: '8px' }}>{item.checked2 ? 'Checked' : 'Unchecked'}</td>
                            <td style={{ border: '1px solid black', padding: '8px' }}>{item.checked3 ? 'Checked' : 'Unchecked'}</td>
